...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="scratch:statement">
    <xsl:text> [</xsl:text>
    <xsl:apply-templates />
    <xsl:text>]</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="//scratch:statement/scratch:block/scratch:next">
    <xsl:text> [</xsl:text>
    <xsl:apply-templates />
    <xsl:text>]</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="//scratch:block//scratch:block">
    ...
    <xsl:text>)</xsl:text>
    <xsl:apply-templates select="scratch:statement" />
